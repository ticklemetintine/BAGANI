...
export class SansinukobMapComponent implements OnInit {
    ...
    private mapAdefaultCoordinates = [
    ...
    private mapBdefaultCoordinates = [
    ...
    private mapCdefaultCoordinates = [
    ...
    private mapDdefaultCoordinates = [
    ...
    private mapEdefaultCoordinates = [
    ...
    @ViewChild('mapFocusA', {
        read: ElementRef
    }) mapFocusA: ElementRef;
    @ViewChild('mapFocusB', {
        read: ElementRef
    }) mapFocusB: ElementRef;
    @ViewChild('mapFocusC', {
        read: ElementRef
    }) mapFocusC: ElementRef;
    @ViewChild('mapFocusD', {
        read: ElementRef
    }) mapFocusD: ElementRef;
    @ViewChild('mapFocusE', {
        read: ElementRef
    }) mapFocusE: ElementRef;

    @Output('onMapClick') onMapClick: EventEmitter < string > = new EventEmitter();
    ...
    ngOnInit() {
        this.renderer.setAttribute(this.mapA.nativeElement, 'coords', this.chunkArray(this.mapAdefaultCoordinates, 2).join(','));
        this.renderer.setAttribute(this.mapB.nativeElement, 'coords', this.chunkArray(this.mapBdefaultCoordinates, 2).join(','));
        this.renderer.setAttribute(this.mapC.nativeElement, 'coords', this.chunkArray(this.mapCdefaultCoordinates, 2).join(','));
        this.renderer.setAttribute(this.mapD.nativeElement, 'coords', this.chunkArray(this.mapDdefaultCoordinates, 2).join(','));
        this.renderer.setAttribute(this.mapE.nativeElement, 'coords', this.chunkArray(this.mapEdefaultCoordinates, 2).join(','));
    }

    onResize($event) {
        let tmp;
        tmp = this.recomputeMap(
            this.mapAdefaultCoordinates
        ).join(',');
        this.renderer.setAttribute(this.mapA.nativeElement, 'coords', tmp);

        tmp = this.recomputeMap(
            this.mapBdefaultCoordinates
        ).join(',');
        this.renderer.setAttribute(this.mapB.nativeElement, 'coords', tmp);

        tmp = this.recomputeMap(
            this.mapCdefaultCoordinates
        ).join(',');
        this.renderer.setAttribute(this.mapC.nativeElement, 'coords', tmp);

        tmp = this.recomputeMap(
            this.mapDdefaultCoordinates
        ).join(',');
        this.renderer.setAttribute(this.mapD.nativeElement, 'coords', tmp);

        tmp = this.recomputeMap(
            this.mapEdefaultCoordinates
        ).join(',');
        this.renderer.setAttribute(this.mapE.nativeElement, 'coords', tmp);
    ...
    initMap() {
        ...
        const clans = [{
                'type': 'trade',
                'el': this.mapFocusA.nativeElement,
                'definition': {
                    defaultW: 319,
                    defaultH: 314,
                    defaultTop: 50,
            ...
            {
                'type': 'desert',
                'el': this.mapFocusB.nativeElement,
                'definition': {
                    defaultW: 390,
                    defaultH: 257,
                    defaultTop: 20,
            ...
            {
                'type': 'farm',
                'el': this.mapFocusC.nativeElement,
                'definition': {
                    defaultW: 319,
                    defaultH: 243,
                    defaultTop: 200,
            ...
            {
                'type': 'forest',
                'el': this.mapFocusD.nativeElement,
                'definition': {
                    defaultW: 286,
                    defaultH: 214,
                    defaultTop: 278,
            ...
            {
                'type': 'fishing',
                'el': this.mapFocusE.nativeElement,
                'definition': {
                    defaultW: 500,
                    defaultH: 265,
                    defaultTop: 421,
